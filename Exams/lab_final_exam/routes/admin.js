...
router.get('/', requireAdmin, async (req, res) => {
  try {
    ...
    res.render('admin/dashboard', {
      title: 'Admin Dashboard - PrettyLittleThing',
...
router.get('/complaints', requireAdmin, async (req, res) => {
  try {
    ...
    // Get complaints with user information
    ...
    res.render('admin/complaints', {
      title: 'All Complaints - Admin - PrettyLittleThing',
    ...
    res.render('admin/complaints', {
      ...
      hasPrevPage: false,
      selectedStatus: '',
      stats: { pending: 0, inProgress: 0, resolved: 0, closed: 0 }
    });
  }
});
...
router.post('/complaints/:id/status', requireAdmin, async (req, res) => {
  try {
    ...
    req.session.message = `Complaint status updated to ${status}`;
