...
router.get('/my-orders', requireAuth, async (req, res) => {
  try {
    ...
    res.render('my-orders', {
      title: 'My Orders - PrettyLittleThing',
      orders: userOrders,
      currentPage: page,
...
router.get('/checkout', async (req, res) => {
  try {
    ...
    res.render('checkout', {
      title: 'Checkout - PrettyLittleThing',
      cart: cart,
      error: req.session.error,
      message: req.session.message,
  ...
  try {
    ...
    if (!order) {
      return res.status(404).render('404', { 
        title: 'Order Not Found - PrettyLittleThing',
    ...
    if (req.session.userId && req.session.userEmail) {
      if (order.customerEmail !== req.session.userEmail) {
        return res.status(403).render('403', {
          title: 'Access Denied - PrettyLittleThing',
    ...
    res.render('order-detail', {
      title: `Order ${order.orderNumber} - PrettyLittleThing`,
      order: order
    });
  } catch (error) {
...
router.get('/', async (req, res) => {
  try {
    ...
    res.render('orders', {
      title: 'Orders - PrettyLittleThing',
      orders: orders,
      currentPage: page,
      totalPages: totalPages,
      hasNextPage: page < totalPages,
